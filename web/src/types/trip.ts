...
export interface PlaceGalleryVariant {
  url: string;
  width: number;
  height: number;
}
...
export interface PlaceGalleryImage {
  ...
  position: number;
  alt_text: string;
  desktop: PlaceGalleryVariant;
  mobile: PlaceGalleryVariant;
  thumb: PlaceGalleryVariant;
}

/** GET /trip/places/{id} 响应。空字段前端按空态隐藏，后端不给假文案。 */
