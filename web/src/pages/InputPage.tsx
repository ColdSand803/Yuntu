/**
 * 云途首页：全屏灵感岛（4段式智能悬浮胶囊 + 自研高定范围日历 + 16座真实POI名城）
 */
...
export default function InputPage() {
  ...
  const stored = useMemo(() => {
    ...
    return pending?.trip_request ?? null;
  }, []);
  ...
  const [preferences, setPreferences] = useState<string[]>(
    storedPrefs ? storedPrefs.filter((p: string) => !["轻松", "适中", "紧凑"].includes(p)) : ["美食", "citywalk", "夜景"]
  );
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden overflow-y-auto bg-gray-950 font-body text-white">
      {/* 1. 全屏沉浸风光巨幕 (固定背景 fixed inset-0，确保移动端平滑滚动且不拦截触控) */}
      <div
        className="fixed inset-0 bg-cover bg-center scale-105 transform-gpu transition-opacity duration-1000 ease-in-out will-change-[opacity,transform] pointer-events-none"
        style={{ backgroundImage: `url('${bgImage}')` }}
      />
      {bgIncoming && (
        <div
          className="fixed inset-0 animate-bg-fade-in bg-cover bg-center scale-105 transform-gpu will-change-[opacity,transform] pointer-events-none"
      ...
      {/* 2. 顶栏标识与真实登录态（全宽两端通栏） */}
      <header className="relative z-30 flex w-full items-center justify-between px-5 py-3.5 sm:px-10 lg:px-14 backdrop-blur-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2 rounded-xl bg-sand-100/80 p-1 border border-sand-200">
                    <button
                      type="button"
                      disabled={people <= 1}
                      onClick={() => setPeople((p: number) => Math.max(1, p - 1))}
                    ...
                    <button
                      type="button"
                      disabled={people >= 20}
                      onClick={() => setPeople((p: number) => Math.min(20, p + 1))}
                      className="flex h-7 w-7 items-center justify-center rounded-lg bg-white text-gray-700 shadow-2xs hover:bg-sand-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                      title="增加人数"
                    >
